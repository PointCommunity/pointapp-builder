...
const REPO = `${OWNER}/pointapp-builder`;
const PROJECT_NUMBER = '5';
const PROJECT_TITLE = 'PointApp Builder';
...
function isBootstrapPr(pr) {
  return BOOTSTRAP_PRS.get(pr.number) === pr.headRefName && pr.baseRefName === 'main';
}
...
export function auditPipelineSnapshot(snapshot) {
  ...
  const bootstrapPrs = prs.filter(isBootstrapPr);
  const pipelinePrs = prs.filter((pr) => !isDependabot(pr) && !isBootstrapPr(pr));

  if (
    snapshot.project?.title !== PROJECT_TITLE ||
    snapshot.project?.public !== false ||
    snapshot.project?.closed !== false ||
    snapshot.project?.shortDescription !== PROJECT_DESCRIPTION
  ) {
  ...
  const statusOptions =
    (snapshot.fields ?? []).find((field) => field.name === 'Status')?.options ?? [];
  for (const [name, expectedDescription] of Object.entries(REQUIRED_STATUS_DESCRIPTIONS)) {
    const option = statusOptions.find((candidate) => candidate.name === name);
    if (option?.description !== expectedDescription) {
      errors.push(`${name} description mismatch: ${option?.description || '(missing)'}`);
    }
  }
  ...
  const liveLabels = new Map((snapshot.availableLabels ?? []).map((label) => [label.name, label]));
  for (const [name, expectedColor] of Object.entries(REQUIRED_LABELS)) {
    const label = liveLabels.get(name);
    if (!label) {
      errors.push(`repository is missing governed label ${name}`);
    } else if ((label.color ?? '').toUpperCase() !== expectedColor) {
      errors.push(`governed label ${name} color mismatch: ${label.color || '(missing)'}`);
    }
  }
  ...
  if (bootstrapPrs.length > 0) {
    const numbers = bootstrapPrs.map((pr) => `#${pr.number}`).join(' and PR ');
    warnings.push(
      `bootstrap exception PR ${numbers} is still open; all later product work requires a governed Issue`,
    );
  }
  ...
  return { errors, warnings, activeItems, pipelinePrs, dependabotPrs, bootstrapPrs };
}
...
export function readLiveSnapshot() {
  ...
  const projectConfiguration = runJson('gh', [
    ...
    'graphql',
    '-f',
    'query=query { organization(login: "PointCommunity") { projectV2(number: 5) { fields(first: 100) { nodes { ... on ProjectV2FieldCommon { name dataType } ... on ProjectV2SingleSelectField { options { name description color } } } } repositories(first: 20) { nodes { nameWithOwner } } workflows(first: 20) { nodes { name enabled } } views(first: 20) { nodes { name layout verticalGroupByFields(first: 5) { nodes { ... on ProjectV2SingleSelectField { name } } } } } } } }',
  ]).data.organization.projectV2;

  return {
    project,
    fields: projectConfiguration.fields.nodes ?? [],
...
async function main() {
  ...
  if (result.errors.length > 0) {
    process.stderr.write(`PointApp Builder pipeline health failed (${result.errors.length}):\n`);
  ...
  process.stdout.write(
    `PointApp Builder pipeline health OK: active=${active}, ${result.pipelinePrs.length} workflow PR(s), ${result.bootstrapPrs.length} bootstrap PR(s), ${result.dependabotPrs.length} Dependabot PR(s).\n`,
  );
}
